    ...
    // Automatenliste
        .state('feederList', {
            ...
            resolve: {
                loadPlugin: ['$ocLazyLoad', '$injector', function($ocLazyLoad, $injector) {
                    return $ocLazyLoad.load([
                        {serie: true, files: ['js/plugins/pdfmake/build/pdfmake.min.js', 'js/plugins/pdfmake/build/vfs_fonts.js', 'js/plugins/ui-grid/ui-grid.js']},
                        {insertBefore: '#loadBefore', files: ['js/plugins/ui-grid/ui-grid.min.css', 'css/diagramm.css']},
        ...
        .state('calvesList', {
            ...
            resolve: {
                loadPlugin: ['$ocLazyLoad', '$injector', function($ocLazyLoad, $injector) {
                    return $ocLazyLoad.load([
                        {serie: true, files: ['js/plugins/pdfmake/build/pdfmake.min.js', 'js/plugins/pdfmake/build/vfs_fonts.js', 'js/plugins/ui-grid/ui-grid.js']},
                        {insertBefore: '#loadBefore', files: ['js/plugins/ui-grid/ui-grid.min.css']},
    ...
    // TodoListe
        .state('todosList', {
            ...
            resolve: {
                loadPlugin: ['$ocLazyLoad', '$injector', '$stateParams', function($ocLazyLoad, $injector, $stateParams) {
                    return $ocLazyLoad.load([
                        { name: 'datePicker', files: ['css/plugins/datapicker/angular-datapicker.css','js/plugins/datapicker/angular-datepicker.js']},
                        {files: ['css/plugins/iCheck/custom.css','js/plugins/iCheck/icheck.min.js']},
                    ...
                    return $ocLazyLoad.load([
                        {
                            ...
                            files: [ 'js/plugins/flot/jquery.flot.js', 'js/plugins/flot/jquery.flot.time.js',
                                  'js/plugins/flot/jquery.flot.tooltip.min.js', 'js/plugins/flot/jquery.flot.spline.js',
                                  'js/plugins/flot/jquery.flot.resize.js', 'js/plugins/flot/jquery.flot.pie.js', 
                                  'js/plugins/flot/curvedLines.js', 'js/plugins/flot/jquery.flot.time.js', 'js/plugins/flot/angular-flot.js']
                        },
                        { name: 'fullCalendar', files: ['js/plugins/fullcalendar/fullcalendar.min.css', 'js/plugins/fullcalendar/fullcalendar.js', 'js/plugins/fullcalendar/calendar.js']},
                    ...
                    loadPlugin: ['$ocLazyLoad', '$rootScope', '$injector', '$stateParams', '$timeout', '$q', '$state', function($ocLazyLoad, $rootScope, $injector, $stateParams, $timeout, $q, $state) {
                        return $ocLazyLoad.load([
                            ...
                            return TaxiTimerServices.getTimer($stateParams.taxi_id).then(function (result) { 
                                if (result.status != 500)  {
                                    return $q.when(result.data.timer);
                                } else {
                                    $timeout(function () {
                                        $state.go('dashboard');
                                    })
                ...
                resolve: {
                    loadPlugin: ['$ocLazyLoad', '$rootScope', '$injector', '$stateParams', '$timeout', '$q', '$state', function($ocLazyLoad, $rootScope, $injector, $stateParams, $timeout, $q, $state) {
                        return $ocLazyLoad.load([
                            ...
                            if (result.status != 500)  {
                                return $q.when(result.data.dosings);
                            } else {
                                $timeout(function () {
                                $state.go('dashboard');
                                })
                        ...
                        return $ocLazyLoad.load([
                            ...
                            return CurvesTaxiServices.getFeedingCurves($stateParams.taxi_id).then(function (result) { 
                               ...
                               if (result.status != 500)  {
                                ...
                                return $q.when(result.data.curves);
                              } else {
                                $timeout(function () {
                                  $state.go('dashboard');
                                })
...
angular
    .module('calfguide')
    .config(config)
    .run(function($rootScope, $state, $http, toaster) {
